...
class SmartDesktopAssistant:
    ...
    def create_panel_ui(self):
        ...
        tk.Label(left_frame, text="🌸 智能助手", font=(self.theme.FONT_MAIN, 13, "bold"), 
                fg=self.theme.FG_WHITE, bg=self.theme.BG_HEADER).pack(side=tk.LEFT)
        
        self.date_label_var = tk.StringVar(value=self.current_view_date)
        
        # 左箭头：前一天
        self._make_cute_btn(left_frame, "◀", self.theme.BTN_SUMMARY, fg_color=self.theme.FG_WHITE,
                           hover_color=self.theme.BTN_SUMMARY_HOVER, command=self.prev_day, 
                           font_size=8, padx=4, pady=1).pack(side=tk.LEFT, padx=(8, 0))
        ...
        date_btn = tk.Button(left_frame, textvariable=self.date_label_var, font=(self.theme.FONT_MONO, 9, "bold"),
                           ...
                           command=self.change_date, padx=8, pady=2, bd=0)
        ...
        # 右箭头：后一天
        self._make_cute_btn(left_frame, "▶", self.theme.BTN_SUMMARY, fg_color=self.theme.FG_WHITE,
                           hover_color=self.theme.BTN_SUMMARY_HOVER, command=self.next_day, 
                           font_size=8, padx=4, pady=1).pack(side=tk.LEFT, padx=(0, 4))
        ...
        self._make_cute_btn(btn_frame, "📊 总结", self.theme.BTN_SUMMARY, hover_color=self.theme.BTN_SUMMARY_HOVER,
                           command=self.generate_summary, font_size=10, padx=20, pady=5).pack(side=tk.LEFT, padx=5, fill=tk.X, expand=True)
        self._make_cute_btn(btn_frame, "🚪 退出", self.theme.BTN_EXIT, hover_color=self.theme.BTN_EXIT_HOVER,
                           command=self.quit_app, font_size=10, padx=20, pady=5).pack(side=tk.RIGHT, padx=5, fill=tk.X, expand=True)
